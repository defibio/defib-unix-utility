...
namespace DefibWindowsUtility
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            CommandDelegator.OnBeatCommand += OnBeatHandlerAsync;
        ...
        private static void OnBeatHandlerAsync(object sender, OnBeatCommandEventArgs arguments)
        {
            if (Storage.ExistsAlias(arguments.Alias))
            {
                HeartBeatTaskAsync(Storage.FetchAlias(arguments.Alias));
        ...
        private static async Task HeartBeatTaskAsync(string key)
        {
            HttpClient defibClient = new HttpClient();
            defibClient.BaseAddress = new System.Uri("https://defib.io/heartbeat/receiver/");

            HttpResponseMessage defibResponse = await defibClient.GetAsync(key);

            string responseFromDefib = await defibResponse.Content.ReadAsStringAsync();
